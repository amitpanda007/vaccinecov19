...
export class HomeComponent implements OnInit {
    ...
    try {
      ...
      if(weeks == 0) {
        ...
        for(let i = 0; i < weeks; i ++) {
          ...
          let newCenters = await this.homeService.getCenterDetailsByPin(pincode, futureDate);
          centerList = await this.aggregateCentersSessions(centerList, newCenters);
        }
        return centerList;
      }
    } catch (error) {
  ...
  async getDistrictCenterInfo(districtId, days) {
    ...
    try {
      ...
      if(weeks == 0) {
        ...
        const newDate = formatDate(today, "dd-MM-yyyy", locale)
        console.log(`Today: ${newDate}`);
        return await this.homeService.getCenterDetailsByDistrict(districtId, newDate);
      }else {
        for(let i = 0; i < weeks; i ++) {
          console.log(i);
          const today = new Date();
          ...
          const futureDate = formatDate(newFutureDate, "dd-MM-yyyy", locale)
          console.log(`Today: ${futureDate}`);
          let newCenters = await this.homeService.getCenterDetailsByDistrict(districtId, futureDate);
          centerList = await this.aggregateCentersSessions(centerList, newCenters);
        }
        return centerList;
      }
  ...
  async onToggleChange() {
    if(this.is18to44) {
      this.centerData = await this.filterByMinMaxAgeLimit(this.centerData, "18", "44");
    }
    
    if(this.is45Plus) {
      this.centerData = await this.filterByMinMaxAgeLimit(this.centerData, "45", "50");
    }
  ...
  async districtSelected($event) {
    ...
    this.centerData = await this.getDistrictCenterInfo(districtId, "28");
  }
}
